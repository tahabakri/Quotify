import { GenerationResult } from '../deepai/types';
...
class ImageStorageService {
  ...
  async saveImage(generatedImage: GenerationResult, quoteId: string): Promise<ImageMetadata> {
    try {
      ...
      const { data: metadata, error: dbError } = await supabase
        ...
        .insert({
          quote_id: quoteId,
          image_url: publicUrl,
          background_type: generatedImage.metadata?.style || 'modern',
          metadata: {
            style: generatedImage.metadata?.style || 'modern',
            mood: generatedImage.metadata?.mood || 'light',
            width: generatedImage.metadata?.width,
            height: generatedImage.metadata?.height,
