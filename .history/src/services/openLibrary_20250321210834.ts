import { Book as SupabaseBook } from './supabase/types';
...
export interface Book extends SupabaseBook {
  coverUrl: string;
  rating: number;
}
...
export async function searchBooks(
  params: SearchParams
): Promise<{ books: Book[]; hasMore: boolean; totalItems: number }> {
  const {
    query = '',
    page = 0,
    limit = 10,
  ...
  try {
    ...
    const books: Book[] = data.docs.map(doc => ({
      ...
      rating: Math.random() * 2 + 3, // Random rating between 3-5 since OpenLibrary doesn't provide ratings
    }));
