...
class GoogleBooksService {
  ...
  private buildSearchQuery({ query, filter, genres }: SearchParams): string {
    ...
    if (filter === 'latest') {
      parts.push('&orderBy=newest');
    }

    if (filter === 'trending') {
      parts.push('&orderBy=relevance');
      // Add search parameters to favor popular books
      parts.push('ratingCount>100'); // Books with significant number of ratings
      parts.push('rating>3.5'); // Books with good ratings
    }
