        ...
        {options.map((option: StyleOption, index: number) => {
          ...
          return (
            <div
              key={option.value}
              role="option"
              aria-selected={(!!isSelected) as boolean | "false" | "true"}
...
export const BackgroundTypeSelector: React.FC<BackgroundTypeSelectorProps> = ({
  ...
  return (
    <div className={`space-y-8 ${className}`}>
      <section>
        <h3 className="text-lg font-semibold mb-4" id="background-type-label">
          Background Type
        </h3>
        <OptionList
          options={BACKGROUND_TYPES}
          selectedValue={value.backgroundType}
          onSelect={(val: string) => updateOptions({ backgroundType: val as BackgroundType })}
          disabled={disabled}
      ...
      <section>
        ...
        <OptionList
          ...
          onSelect={(val: string) => updateOptions({ mood: val as BackgroundMood })}
          disabled={disabled}
          id="mood-list"
          labelId="mood-label"
        />
      ...
      <section>
        ...
        <OptionList
          ...
          onSelect={(val: string) => updateOptions({ style: val as BackgroundStyle })}
          disabled={disabled}
          id="style-list"
          labelId="style-label"
        />
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold" id="custom-adjustments-label">Custom Adjustments</h3>
          <button
            onClick={() => setIsCustomPromptVisible(prev => !prev)}
            className="text-blue-500 hover:text-blue-600 text-sm font-medium"
            aria-expanded={(!!isCustomPromptVisible) as boolean | "false" | "true"}
            aria-controls="custom-prompt-section"
            type="button"
          >
            {isCustomPromptVisible ? 'Hide' : 'Show'}
          </button>
        </div>
