...
export function SearchProvider({ children }: { children: React.ReactNode }) {
  ...
  const performSearch = useCallback(async (params: SearchParams) => {
    ...
    try {
      ...
      if ('books' in results) {
        // Handle Google Books API response
        setBooks(results.books);
        setHasMore(results.hasMore);
        setTotalItems(results.totalItems);
      } else {
        // Handle Open Library API response
        setBooks(results);
        setHasMore(results.length === 24); // Assuming 24 is the page size
      ...
      try {
        ...
        if ('books' in fallbackResults) {
          ...
          setHasMore(fallbackResults.length === 24);
          setTotalItems(fallbackResults.length);
        }
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if ('books' in results) {
        ...
        setTotalItems(prev => prev + results.length);
      }
